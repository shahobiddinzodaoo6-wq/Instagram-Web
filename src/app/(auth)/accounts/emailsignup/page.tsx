...
const Page = () => {
    ...
    const handleSubmit = async (event) => {
        ...
        if (event.target["inpPassword"].value != event.target["inpConfirmPassword"].value) {
            sileo.error({ title: "Ошибка", description: "Пароли не совпадают", position: "top-center" });
            setIsLoading(false)
            return;
        }
        const obj = {
        ...
        try {
            ...
            if (data.statusCode == 200 || data.data) {
                sileo.success({ title: "Аккаунт создан!", description: "Добро пожаловать. Пожалуйста, войдите.", position: "top-center" });
                router.push("/accounts/login")
            }
        } catch (error) {
            console.error(error);
            sileo.error({ title: "Ошибка регистрации", description: "Этот email или имя пользователя могут быть уже заняты.", position: "top-center" });
        } finally {
            setIsLoading(false)
        }
    }
            ...
            <div className="hidden lg:flex flex-1 flex-col items-center justify-center p-8 bg-[#fafafa] border-r border-[#dbdbdb]">
                <div className="max-w-[450px] text-center flex flex-col items-center">
                    ...
                    <h1 className="text-[26px] font-bold leading-tight mb-4 px-4">
                        Зарегистрируйтесь, чтобы смотреть фото и видео ваших <span className="bg-gradient-to-r from-[#f09433] via-[#e6683c] to-[#bc1888] bg-clip-text text-transparent">друзей</span>.
                    </h1>
            ...
            <div className="w-full lg:w-[450px] flex flex-col items-center justify-center p-6 lg:p-10 h-full bg-white relative">
                <div className="w-full max-w-[350px] flex flex-col h-full py-4 justify-center">
                    <div className="flex items-center gap-2 mb-4">
                        <Link href="/accounts/login" className="hover:opacity-70">
                    ...
                    <form onSubmit={handleSubmit} className="flex flex-col gap-2 no-scrollbar overflow-y-auto max-h-[60vh] py-1 px-0.5">
                        <div className="relative group">
                            <input type="text" name="inpEmail" placeholder="Моб. номер или эл. адрес" className="w-full bg-[#fafafa] border border-[#dbdbdb] rounded-[8px] px-3 py-3 text-sm focus:outline-none focus:border-[#a8a8a8]" id="inpEmail" required />
                        </div>
                        <div className="relative group">
                            <input type="text" name="inpFullName" placeholder="Имя и фамилия" className="w-full bg-[#fafafa] border border-[#dbdbdb] rounded-[8px] px-3 py-3 text-sm focus:outline-none focus:border-[#a8a8a8]" id="inpFullName" required />
                        </div>
                        <div className="relative group">
                            <input type="text" name="inpName" placeholder="Имя пользователя" className="w-full bg-[#fafafa] border border-[#dbdbdb] rounded-[8px] px-3 py-3 text-sm focus:outline-none focus:border-[#a8a8a8]" id="inpName" required />
                        </div>
                        <div className="relative group">
                            <input type="password" name="inpPassword" placeholder="Пароль" className="w-full bg-[#fafafa] border border-[#dbdbdb] rounded-[8px] px-3 py-3 text-sm focus:outline-none focus:border-[#a8a8a8]" id="inpPassword" required />
                        </div>
                        <div className="relative group">
                            <input type="password" name="inpConfirmPassword" placeholder="Подтвердите пароль" className="w-full bg-[#fafafa] border border-[#dbdbdb] rounded-[8px] px-3 py-3 text-sm focus:outline-none focus:border-[#a8a8a8]" id="inpConfirmPassword" required />
                        </div>

                        <p className="text-[11px] text-[#8e8e8e] text-center my-2 leading-tight">
                            Регистрируясь, вы принимаете наши Условия, Политику конфиденциальности и Политику в отношении файлов cookie.
                        </p>

                        <button 
                            type="submit" 
                            disabled={isLoading}
                            className={`bg-[#0095f6] hover:bg-[#1877f2] text-white font-semibold py-2.5 rounded-[12px] text-sm transition-colors ${isLoading ? 'opacity-70' : ''}`}
                        >
                    ...
                    <div className="mt-4 pt-4 border-t border-[#dbdbdb] text-center">
                        <span className="text-sm">Есть аккаунт? </span>
                        <Link href="/accounts/login" className="text-sm font-semibold text-[#0095f6] hover:opacity-70">
                            Вход
                        </Link>
                    </div>
